...
// RegisterCron registers the inventory discovery cron workflow.
func (api *API) RegisterCron() error {
	ManagerAccess.Data.EB.Log.Info().Msg("ComputeAllocation: Registering Inventory Discovery Cron")

	workflowID := "inventory-compute-allocation-" + ManagerAccess.Conf.EB.Temporal.TemporalSubscribeNamespace
	...
	ManagerAccess.Data.EB.Log.Info().Str("Schedule", cronSchedule).Msg("ComputeAllocation: Inventory Discovery Cron Schedule")
	...
	we, err := ManagerAccess.Data.EB.Managers.Workflow.Temporal.Subscriber.ExecuteWorkflow(
		context.Background(),
		workflowOptions,
		sww.DiscoverComputeAllocationInventory,
	)
	if err != nil {
		ManagerAccess.Data.EB.Log.Error().Err(err).Msg("ComputeAllocation: Error registering Inventory Collect/Publish Cron")
		return err
	}
	...
	ManagerAccess.Data.EB.Log.Info().Interface("Workflow ID", wid).Msg("ComputeAllocation: successfully registered Inventory Collect/Publish Cron")
